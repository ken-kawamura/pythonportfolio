...
def red_detect(img):
    ...
    # 赤色のHSVの値域1
    hsv_min = np.array([15,127,220])
    hsv_max = np.array([30,255,255])
    mask1 = cv2.inRange(hsv, hsv_min, hsv_max)

    # 赤色のHSVの値域2
    #hsv_min = np.array([150,127,220])
...
def main():
    ...
    writer = cv2.VideoWriter('capture_kaiseki.mp4', cv2.VideoWriter_fourcc(*'mp4v'),25, (width, height))
    ...
    while(cap.isOpened()):
        ...
        # CSVファイルに保存
        np.savetxt('kaiseki.csv', np.array(data), delimiter=",")
